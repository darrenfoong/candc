...
public class Combinators {
	...
	Unify unification;
	RuleInstancesParams ruleInstancesParams; // settings determine which
	// seen_rule instances are used
	RuleInstances coordinationRuleInstances;
	RuleInstances backwardCompRuleInstances;

	public Combinators(boolean eisnerNormalForm, Categories categories,
		...
		if (ruleInstancesParams.getConj()) {
			coordinationRuleInstances = new RuleInstances(
					ruleInstancesParams.getDirectory() + "/conj_rule_instances",
					categories);
		}
		if (ruleInstancesParams.getBackwardComp()) {
			backwardCompRuleInstances = new RuleInstances(
					ruleInstancesParams.getDirectory()
					+ "/backward_comp_rule_instances", categories);
		}
	}

	private boolean application(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, boolean FWD_APP,
			ArrayList<SuperCategory> results, Sentence sentence) {
		/*
		...
		if (FWD_APP) {
			if (!unification
					.unify(leftSuperCat.cat.argument, rightSuperCat.cat)) {
				return false;
			}
		} else {
			if (!unification
					.unify(leftSuperCat.cat, rightSuperCat.cat.argument)) {
				return false;
			}
		}
		 ...
		 */
		if (FWD_APP) {
			leftSuperCat.cat.result.reorderVariables(unification.trans1,
					unification.seen, unification.order);
		} else {
			rightSuperCat.cat.result.reorderVariables(unification.trans2,
					unification.seen, unification.order);
		}
		...
		Category resultCat;
		if (FWD_APP) {
			resultCat = Category.TransVariable(leftSuperCat.cat.result,
					unification.trans1, unification.feature);
		} else {
			resultCat = Category.TransVariable(rightSuperCat.cat.result,
					unification.trans2, unification.feature);
		}

		SuperCategory resultSuperCat;
		if (FWD_APP) {
			resultSuperCat = SuperCategory.BinaryCombinator(resultCat,
					SuperCategory.FWD_APP, leftSuperCat, rightSuperCat,
					unification, sentence);
		} else {
			resultSuperCat = SuperCategory.BinaryCombinator(resultCat,
					SuperCategory.BWD_APP, leftSuperCat, rightSuperCat,
					unification, sentence);
		}
	...
	private boolean composition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, short compositionType,
			ArrayList<SuperCategory> results, Sentence sentence) {
		/*
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			if (!unification.unify(leftSuperCat.cat.argument,
					rightSuperCat.cat.result)) {
				return generalisedForwardComposition(leftSuperCat,
						rightSuperCat, results, sentence);
			}
		} else { // applies to both BWD_COMP and BWD_CROSS
			if (!unification.unify(leftSuperCat.cat.result,
					rightSuperCat.cat.argument)) {
				if (compositionType == SuperCategory.BWD_CROSS) {
					return generalisedBackwardCrossComposition(leftSuperCat,
							rightSuperCat, results, sentence);
				} else {
					return generalisedBackwardComposition(leftSuperCat,
							rightSuperCat, results, sentence);
				}
			}
		}
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			leftSuperCat.cat.result.reorderVariables(unification.trans1,
					unification.seen, unification.order);
			rightSuperCat.cat.argument.reorderVariables(unification.trans2,
					unification.seen, unification.order);
		} else { // applies to both BWD_COMP and BWD_CROSS
			rightSuperCat.cat.result.reorderVariables(unification.trans2,
					unification.seen, unification.order);
			leftSuperCat.cat.argument.reorderVariables(unification.trans1,
					unification.seen, unification.order);
		}
		/*
		 ...
		 */
		if (var1 != VarID.NONE
				&& unification.seen[unification.trans1[var1]] != VarID.NONE) {
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			leftCat = Category.TransVariable(leftSuperCat.cat.result,
					unification.trans1, unification.feature);
			rightCat = Category.TransVariable(rightSuperCat.cat.argument,
					unification.trans2, unification.feature);
		} else { // applies to both BWD_COMP and BWD_CROSS
			rightCat = Category.TransVariable(leftSuperCat.cat.argument,
					unification.trans1, unification.feature);
			leftCat = Category.TransVariable(rightSuperCat.cat.result,
					unification.trans2, unification.feature);
		}
		...
		resultCat = new Category(leftCat, slash, rightCat, var1, (short) (0),
				(short) (0));
		resultSuperCat = SuperCategory.BinaryCombinator(resultCat,
				compositionType, leftSuperCat, rightSuperCat, unification,
				sentence);
		if (resultSuperCat != null) {
			results.add(resultSuperCat);
			if (compositionType == SuperCategory.FWD_COMP) {
	 ...
	 */
	private boolean generalisedCompositionB2(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, short compositionType,
			ArrayList<SuperCategory> results, Sentence sentence) {
		if (compositionType == SuperCategory.FWD_COMP) {
			if (!unification.unify(leftSuperCat.cat.argument,
					rightSuperCat.cat.result.result)) {
				return false;
			}
		} else { // BWD_CROSS
			if (!unification.unify(leftSuperCat.cat.result.result,
					rightSuperCat.cat.argument)) {
				return false;
			}
		}
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			innerCat.argument.reorderVariables(unification.trans2,
					unification.seen, unification.order);
		} else {
			// BWD_CROSS case
			innerCat.argument.reorderVariables(unification.trans1,
					unification.seen, unification.order);
		}

		if (compositionType == SuperCategory.FWD_COMP) {
			if (var != VarID.NONE
					&& unification.seen[unification.trans2[var]] != VarID.NONE) {
				unification.seen[unification.trans2[var]] = ++unification.order.value;
			}
		} else { // BWD_CROSS case
			if (var != VarID.NONE
					&& unification.seen[unification.trans1[var]] != VarID.NONE) {
		...
		byte innerVar;
		if (compositionType == SuperCategory.FWD_COMP) {
			innerResult = Category.TransVariable(leftSuperCat.cat.result,
					unification.trans1, unification.feature);
			innerArgument = Category.TransVariable(
					rightSuperCat.cat.result.argument, unification.trans2,
					unification.feature);
			innerVar = unification.trans2[rightSuperCat.cat.result.var];
			newResult = new Category(innerResult,
					rightSuperCat.cat.result.slash, innerArgument, innerVar,
					rightSuperCat.cat.result.relID,
					rightSuperCat.cat.result.lrange);
			newArgument = Category.TransVariable(rightSuperCat.cat.argument,
					unification.trans2, unification.feature);
		} else { // BWD_CROSS case
			innerResult = Category.TransVariable(rightSuperCat.cat.result,
					unification.trans2, unification.feature);
			innerArgument = Category.TransVariable(
					leftSuperCat.cat.result.argument, unification.trans1,
					unification.feature);
			innerVar = unification.trans1[leftSuperCat.cat.result.var];
			newResult = new Category(innerResult,
					leftSuperCat.cat.result.slash, innerArgument, innerVar,
					leftSuperCat.cat.result.relID,
					leftSuperCat.cat.result.lrange);
			newArgument = Category.TransVariable(leftSuperCat.cat.argument,
					unification.trans1, unification.feature);
		}
		var1 = unification.trans1[var1]; // new variable for the "outer"
		// variable on the new category
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			newCategory = new Category(newResult, rightSuperCat.cat.slash,
					newArgument, var1, rightSuperCat.cat.relID,
					rightSuperCat.cat.lrange);
		} else {
			// BWD_CROSS case
			newCategory = new Category(newResult, leftSuperCat.cat.slash,
					newArgument, var1, leftSuperCat.cat.relID,
					rightSuperCat.cat.lrange);
		}

		if (compositionType == SuperCategory.FWD_COMP) {
			resultSuperCat = SuperCategory.BinaryCombinator(newCategory,
					(short) (SuperCategory.FWD_COMP | SuperCategory.RECURSIVE),
					leftSuperCat, rightSuperCat, unification, sentence);
		} else {
			// BWD_CROSS case
			resultSuperCat = SuperCategory
					.BinaryCombinator(
							newCategory,
							(short) (SuperCategory.BWD_CROSS | SuperCategory.RECURSIVE),
							leftSuperCat, rightSuperCat, unification, sentence);
		}
		if (resultSuperCat != null) {
			results.add(resultSuperCat);
	 ...
	 * applies to forward and backward cross, but not backward, comp
	 */
	private boolean generalisedCompositionB3(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, short compositionType,
			ArrayList<SuperCategory> results, Sentence sentence) {
		if (compositionType == SuperCategory.FWD_COMP) {
			if (!unification.unify(leftSuperCat.cat.argument,
					rightSuperCat.cat.result.result.result)) {
				return false;
			}
		} else { // BWD_CROSS
			if (!unification.unify(leftSuperCat.cat.result.result.result,
					rightSuperCat.cat.argument)) {
				return false;
			}
		}
		...
		if (compositionType == SuperCategory.FWD_COMP) {
			innerInnerResult = Category.TransVariable(leftSuperCat.cat.result,
					unification.trans1, unification.feature);
			innerInnerArgument = Category.TransVariable(
					rightSuperCat.cat.result.result.argument,
					unification.trans2, unification.feature);
			innerInnerVar = unification.trans2[rightSuperCat.cat.result.result.var];
			innerResult = new Category(innerInnerResult,
					rightSuperCat.cat.result.result.slash, innerInnerArgument,
					innerInnerVar, rightSuperCat.cat.result.result.relID,
					rightSuperCat.cat.result.result.lrange);
			innerArgument = Category.TransVariable(
					rightSuperCat.cat.result.argument, unification.trans2,
					unification.feature);
			innerVar = unification.trans2[rightSuperCat.cat.result.var];
			...
			innerInnerResult = Category.TransVariable(rightSuperCat.cat.result,
					unification.trans2, unification.feature);
			innerInnerArgument = Category.TransVariable(
					leftSuperCat.cat.result.result.argument,
					unification.trans1, unification.feature);
			innerInnerVar = unification.trans1[leftSuperCat.cat.result.result.var];
			innerResult = new Category(innerInnerResult,
					leftSuperCat.cat.result.result.slash, innerInnerArgument,
					innerInnerVar, leftSuperCat.cat.result.result.relID,
					leftSuperCat.cat.result.result.lrange);
			innerArgument = Category.TransVariable(
					leftSuperCat.cat.result.argument, unification.trans1,
					unification.feature);
			innerVar = unification.trans1[leftSuperCat.cat.result.var];
	...
	public boolean forwardApplication(SuperCategory leftSuperCat,
		...
		if (forwardAppReject(leftSuperCat, rightSuperCat)) {
			...
			return application(leftSuperCat, rightSuperCat, true, results,
					sentence); // boolean indicates FWD_APP
		}
	}

	public boolean backwardApplication(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		if (backwardAppReject(leftSuperCat, rightSuperCat)) {
			...
			return application(leftSuperCat, rightSuperCat, false, results,
					sentence); // boolean indicates BWD_APP
		}
	}

	public boolean forwardComposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		if (forwardCompReject(leftSuperCat, rightSuperCat)) {
			return false;
		} else {
			return composition(leftSuperCat, rightSuperCat,
					SuperCategory.FWD_COMP, results, sentence);
		}
	}

	private boolean generalisedForwardComposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		...
		if (generalisedForwardCompB3Reject(leftSuperCat, rightSuperCat)) {
			...
			return generalisedCompositionB3(leftSuperCat, rightSuperCat,
					SuperCategory.FWD_COMP, results, sentence);
		}
	}

	public boolean backwardComposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		if (backwardCompReject(leftSuperCat, rightSuperCat)) {
			...
			return composition(leftSuperCat, rightSuperCat,
					SuperCategory.BWD_COMP, results, sentence);
		}
	}
	 ...
	 */
	private boolean generalisedBackwardComposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		...
		SuperCategory resultSuperCat = SuperCategory.BinaryCombinator(
				leftSuperCat.cat,
				(short) (SuperCategory.BWD_COMP | SuperCategory.RECURSIVE),
				leftSuperCat, rightSuperCat, unification, sentence);
		/*
	...
	public boolean backwardCrossComposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results,
			Sentence sentence) {
		if (backwardCrossReject(leftSuperCat, rightSuperCat)) {
			...
			return composition(leftSuperCat, rightSuperCat,
					SuperCategory.BWD_CROSS, results, sentence);
		}
	}

	private boolean generalisedBackwardCrossComposition(
			SuperCategory leftSuperCat, SuperCategory rightSuperCat,
			ArrayList<SuperCategory> results, Sentence sentence) {
	...
	public boolean coordination(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results) {
		...
		results.add(SuperCategory.Coordination(cat, SuperCategory.CONJ,
	 ...
	 */
	public boolean funnyConj(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results) {
		if (!leftSuperCat.cat.isConj() || !rightSuperCat.cat.isN()) {
			return false;
		}

		results.add(SuperCategory.Punct(rightSuperCat.cat,
				SuperCategory.FUNNY_CONJ, leftSuperCat, rightSuperCat,
				rightSuperCat));
	...
	public boolean apposition(SuperCategory leftSuperCat,
			SuperCategory rightSuperCat, ArrayList<SuperCategory> results) {
		if ((leftSuperCat.cat.isNP() && rightSuperCat.cat.isNP())
				|| (leftSuperCat.cat.isSdcl() && rightSuperCat.cat.isSdcl())) {
			results.add(SuperCategory.Apposition(SuperCategory.APPO,
					leftSuperCat, rightSuperCat));
	...
	private boolean forwardAppReject(SuperCategory left, SuperCategory right) {
		return left.cat.notFwd() || right.coordinatedOrTypeRaised()
				|| (eisnerNormalForm && left.forwardComp());
	}

	private boolean backwardAppReject(SuperCategory left, SuperCategory right) {
		return right.cat.notBwd() || left.coordinatedOrTypeRaised()
	...
	private boolean forwardCompReject(SuperCategory left, SuperCategory right) {
		return left.cat.notFwd() || right.cat.notFwd()
				|| (eisnerNormalForm && left.forwardComp());
	}

	private boolean generalisedForwardCompB2Reject(SuperCategory left,
			SuperCategory right) {
		/*
		 ...
		 */
		return !left.cat.argument.isSbNP() || right.cat.result.notFwd()
				|| right.cat.result.result.isAtomic()
				|| right.cat.result.result.result.hasFeatureVar();
	}

	private boolean generalisedForwardCompB3Reject(SuperCategory left,
			SuperCategory right) {
		/*
		 ...
		 * category has enough "depth")
		 */
		return right.cat.result.result.notFwd()
				|| right.cat.result.result.result.isAtomic()
				|| right.cat.result.result.result.result.hasFeatureVar();
	}
	 ...
	 */
	private boolean generalisedBackwardCompReject(SuperCategory left,
			SuperCategory right) {
	...
	private boolean backwardCrossReject(SuperCategory left, SuperCategory right) {
		return left.cat.notFwd() || right.cat.notBwd() || right.coordinated()
				|| right.cat.isArgNorNP();
	}

	private boolean generalisedBackwardCrossB2Reject(SuperCategory left,
			SuperCategory right) {
		return !right.cat.argument.isSbNP() || left.cat.result.notFwd()
				|| left.cat.result.result.isAtomic()
				|| left.cat.result.result.result.hasFeatureVar();
	}

	private boolean generalisedBackwardCrossB3Reject(SuperCategory left,
			SuperCategory right) {
		return left.cat.result.result.notFwd()
				|| left.cat.result.result.result.isAtomic()
				|| left.cat.result.result.result.result.hasFeatureVar();
	}
	...
	private boolean coordinationReject(SuperCategory left, SuperCategory right) {
		 ...
		 */
		if (ruleInstancesParams.getConj()) {
			return !left.cat.isConj()
					|| !coordinationRuleInstances.contains(left.cat, right.cat);
		} else {
			return !left.cat.isConj() || right.coordinatedOrTypeRaised()
					|| right.cat.isConjbConj();
		}
	}
}
